...
export default {
  ...
  data(){
    return {
      ...
      cur_row: {},
      dialogVisible: false,
      today: new Date().toLocaleDateString() + "库存快照",
      json_fields: {
        '行号':'line',
        '商品名称':'name',
        '型号':'type',
    ...
    exportAsExcel() {
      ...
      getWarehouseCounting().then(_res => {
        ...
        for(var i=0; i<_res.result.length; i++){
          details.push({
            'line':l,
            'name':_res.result[i].product.name,
            'type':_res.result[i].product.type,
            'quantity':_res.result[i].product.quantity,
            'purchasePrice':_res.result[i].product.quantity,
            'batchId':_res.result[i].batchId,
            'id':_res.result[i].id,
            'time':"",
          })
          l++;
